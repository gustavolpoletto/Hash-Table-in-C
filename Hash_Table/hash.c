...
void insert(hash_table** table)
{
    ...
    {
        hash_table* new_table = resize_hashTable(*table);
        if(new_table)
            *table = new_table;
    }
}
...
{
    return (float)table->count / table->size > 0.7;
}
...
hash_table* resize_hashTable (hash_table* old_table)
{
    ...
    {
        node* ptr = old_table->buckets[i];

        while(ptr)
        {
           insert_by_ptr(new_table, ptr->p);
           ptr = ptr->next;
        }
    }
